...
master.dir <- "Simulations_Main_manyCov"
...
check_create.dir(sprintf("%s/%s", master.dir, "LF-AFT_ll__IF-cd"))
check_create.dir(sprintf("%s/%s", master.dir, "LF-AFT_ll__IF-cdmc"))
check_create.dir(sprintf("%s/%s", master.dir, "LF-Cox_wb__IF-cd"))
check_create.dir(sprintf("%s/%s", master.dir, "LF-Cox_wb__IF-cdmc"))
...
sim.results.subset.AFT_LL <- sim.results[sim.results$DGP %in% c(21:26, 33:35), ]
results2latex(sim.results.subset.AFT_LL,
              "More IF. Results AFT model",
              "tab: moreIF, AFT")

# Results for Cox model
sim.results.subset.Cox_wb <- sim.results[sim.results$DGP %in% c(27:32, 36:38), ]
results2latex(sim.results.subset.Cox_wb,
              "More IF. Results Cox model",
              "tab: moreIF, Cox")
