...
if (on.VSC) {
  ...
  # Set master directory
  master.dir <- "Simulations main manyCov"
  check_create.dir(master.dir)
  
  # Arguments supplied to the function (for testing purposes)
  args <- c(5000141,1000,8,27,2,1,4,4)
}
...
n.cov <- as.numeric(args[7])
inst.func.family.idx <- as.numeric(args[8])
...
# Generate data
data <- generateData_simManyCov(beta.true, n, n.cov, options, H0.inv,
                                plot.data = (verbose >= 4))
...
defaults <- list(Kbar = 3,
                 ...
                 Gc = "spline",
                 degree = 3)
results.idx <- unique(((seed - 5000123) %/% 10) %% 50)
